...
pub async fn on_cs_factory_op(ctx: &mut NetContext<'_>, req: CsFactoryOp) -> ScFactoryOpRet {
    ...
    // The client sometimes omits the region name (empty string) to mean
    // "whatever region I'm currently in". Downstream handlers look the
    // region up by exact name, so an empty name would otherwise miss
    // the map and fail with RegionNotFound. Resolve it to the player's
    // current region here so every op below sees a real region name.
    ...
    match (op_type, payload) {
        ...
        (FactoryOpType::SetTravelPoleDefaultNext, Some(CsOp::SetTravelPoleDefaultNext(p))) => {
            special::handle_set_travel_pole_next(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::UseHealTowerPoint, Some(CsOp::UseHealTowerPoint(p))) => {
            special::handle_use_heal_tower(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::CacheTransportEnable, Some(CsOp::CacheTransportEnable(p))) => {
            cache_transport::handle_enable(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::CacheTransportTransfer, Some(CsOp::CacheTransportTransfer(p))) => {
            cache_transport::handle_transfer(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemCacheToCache, Some(CsOp::MoveItemCacheToCache(p))) => {
            gridbox::handle_move_cache_to_cache(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemBagToCache, Some(CsOp::MoveItemBagToCache(p))) => {
            gridbox::handle_move_bag_to_cache(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemCacheToBag, Some(CsOp::MoveItemCacheToBag(p))) => {
            gridbox::handle_move_cache_to_bag(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemDepotToCache, Some(CsOp::MoveItemDepotToCache(p))) => {
            gridbox::handle_move_depot_to_cache(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemCacheToDepot, Some(CsOp::MoveItemCacheToDepot(p))) => {
            gridbox::handle_move_cache_to_depot(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemConveyorToBag, Some(CsOp::MoveItemConveyorToBag(p))) => {
            gridbox::handle_move_conveyor_to_bag(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::GridBoxInnerMove, Some(CsOp::GridBoxInnerMove(p))) => {
            gridbox::handle_inner_move(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::GridBoxInnerSplit, Some(CsOp::GridBoxInnerSplit(p))) => {
            gridbox::handle_inner_split(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemBagToGridBox, Some(CsOp::MoveItemBagToGridBox(p))) => {
            gridbox::handle_move_bag_to_gridbox(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemGridBoxToBag, Some(CsOp::MoveItemGridBoxToBag(p))) => {
            gridbox::handle_move_gridbox_to_bag(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemDepotToGridBox, Some(CsOp::MoveItemDepotToGridBox(p))) => {
            gridbox::handle_move_depot_to_gridbox(ctx, req.index, region_name, p).await
        }
        ...
        (FactoryOpType::MoveItemGridBoxToDepot, Some(CsOp::MoveItemGridBoxToDepot(p))) => {
            gridbox::handle_move_gridbox_to_depot(ctx, req.index, region_name, p).await
        }
